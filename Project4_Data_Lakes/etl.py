...
def process_song_data(spark, input_data, output_data):
    ...
    artists_table = df.select(col("artist_id"),col("name"),col("location"),col("lattitude"),col("longtitude")).distinct()
    
    # write artists table to parquet files
    artists_table.write.parquet("{}artists/artists_table.parquet".format(output_data))
    df.getOrCreateTempView("song_table")
...
def process_log_data(spark, input_data, output_data):
    ...
    song_df = spark.sql("select distict song_id, artist_id, artist_name from song_table")
    ...
    songplays_table.write.partitionBy("year","month").parquet("{}song_plays/song_plays_table.parquet".format(output_data))
...
def main():
    ...
    output_data = "s3://aws-emr-resources-449629588483-us-west-2/sparkify/"
